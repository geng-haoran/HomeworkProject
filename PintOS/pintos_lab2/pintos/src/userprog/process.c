...
{
  // printf("??????????????????????haoran: start_process: file_name = %s \n", file_name);
  
  char *fn_copy, *fn_copy_; // haoran: fn_copy_ is used to store 
    //the original file_name, which will be used in start_process
  tid_t tid;  // haoran: thread id
  ...
  char *tmp_p;
  char *fn_name = strtok_r(fn_copy, " ", &tmp_p);
  // printf("??????????????????????haoran: start_process: fn_name = %s \n", fn_name);

  /* Create a new thread to execute FILE_NAME. */
  // printf("?????????????????!!!!!!!!!!!!!!!!!!!!!!!!!!!!!?");
  // printf("$$$$$$$$$$$$$$ %s \n", fn_name);
  // printf("$$$$$$$$$$$$$$ %s \n", fn_copy_);
  tid = thread_create (fn_name, PRI_DEFAULT, start_process, fn_copy_);
  
  palloc_free_page (fn_copy); // haoran: free the memory
  if (tid == TID_ERROR)
  {
    palloc_free_page (fn_copy_); // haoran: free the memory
  }

  sema_down(&thread_current()->sema_exec); // haoran: wait for the child to finish loading
...
start_process (void *file_name_)
{
  ...
  char *tmp_p, *fn_name, *file_copy;
  file_copy = palloc_get_page(0);
  strlcpy(file_copy, file_name, PGSIZE); // haoran: copy the file name
  fn_name = strtok_r(file_name, " ", &tmp_p); // haoran: get the fn name
  ...
  // printf("@@@@@@@@@@@@@@@@@@ %s \n", fn_name);
  // printf("@@@@@@@@@@@@@@@@@@ %s \n", file_name);
  success = load (fn_name, &if_.eip, &if_.esp);
  ...
  for (char * token = strtok_r (file_copy, " ", &tmp_p); \
    token != NULL; token = strtok_r (NULL, " ", &tmp_p)) 
  {
  ...
  // Push the address of argv[0],argc, fake "return address"(0).
  if_.esp -= ptr_size;
  ...
  *(int *)if_.esp = argc;
  ...
  if_.esp -= ptr_size;
  memset(if_.esp, 0, ptr_size); // fake return address

  // printf("??????????????????????????????????????????????????STACK SET. ESP: %p\n", if_.esp);
  // hex_dump((uintptr_t)if_.esp, if_.esp, 100, true); // haoran: print the stack
...
process_exit (void)
{
  ...
  printf("%s: exit(%d)", cur->name, cur->exit_code);
