...
def pgd_train(
    ...
    for imgs, labels in tqdm.tqdm(train_loader):
        ...
        for t in range(steps):
            ...
            if args.orthogonal:
                loss_adv = criterion(output_adv, labels) + regularizationTerm(
                    model, "orthogonal"
                )
            elif args.spectral:
                loss_adv = criterion(output_adv, labels) + regularizationTerm(
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--lr", "-l", type=float, default=1e-4, help="Learning rate"
    )
    arg_parser.add_argument(
    ...
    arg_parser.add_argument(
        "--batchsize", "-b", type=int, default=200, help="batch size"
    )
    ...
    arg_parser.add_argument(
        "--attack", "-attack", action="store_true", help="whether to train with PGD"
    )
    arg_parser.add_argument(
        "--quant_model",
        "-quant_model",
        action="store_true",
        help="whether to use quant model",
    )
    arg_parser.add_argument(
        "--small_model",
        "-small_model",
        action="store_true",
        help="whether to use small model",
    )
    arg_parser.add_argument(
        "--quantization",
        "-quantization",
    ...
    arg_parser.add_argument(
        ...
        "-orthogonal",
        action="store_true",
        help="whether to use orthogonal regularization",
    )
    arg_parser.add_argument(
        "--spectral",
        "-spectral",
        action="store_true",
        help="whether to use spectral regularization",
    )
    arg_parser.add_argument(
        "--RSE", "-RSE", action="store_true", help="whether to use RSE noise"
    )
