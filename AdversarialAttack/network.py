...
def regularizationTerm(model, reg_type, beta=1e-4):
    ...
    if reg_type == "orthogonal":
        loss_orth = torch.tensor(0.0, dtype=torch.float32).cuda()
        ...
        for name, param in model.named_parameters():
            if "weight" in name and param.requires_grad and len(param.shape) == 4:
                ...
                diag = torch.eye(
                    weight_squared.shape[0], dtype=torch.float32
                )  # (N * C) * H * H
...
class ConvNet_quant(nn.Module):
    ...
    def forward(self, x, quant=True):
        ...
        if quant:
            conv1_scale = (
                torch.max(
                    torch.max(torch.max(torch.abs(self.conv1.weight), dim=1)[0], dim=1)[
                        0
                    ],
                    dim=1,
                )[0]
            ...
            self.conv1_int.weight = torch.nn.Parameter(
                ((self.conv1.weight / conv1_scale).int() * conv1_scale).float()
            )
            conv2_scale = (
                torch.max(
                    torch.max(torch.max(torch.abs(self.conv2.weight), dim=1)[0], dim=1)[
                        0
                    ],
                    dim=1,
                )[0]
            ...
            self.conv2_int.weight = torch.nn.Parameter(
                ((self.conv2.weight / conv2_scale).int() * conv2_scale).float()
            )
            Linear_scale = torch.max(torch.abs(self.Linear.weight)) / (2 ** (8 - 1) - 1)
            self.Linear_int.weight = torch.nn.Parameter(
                ((self.Linear.weight / Linear_scale).int() * Linear_scale).float()
            )
